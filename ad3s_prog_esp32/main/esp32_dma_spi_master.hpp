// Copyright (c) 2019 hideakitai
// SPDX-License-Identifier: MIT
//
// Vendored from https://github.com/hideakitai/arduino-esp32-dma-spi (MIT).
// Modified for ad3s_prog_esp32: added #include "ad3s_types.hpp" and remapped
// SPI pins/GPIO constants to this project's pin table.
#pragma once
#ifndef ESP32_DMA_SPI_MASTER_HPP
#define ESP32_DMA_SPI_MASTER_HPP
...
static constexpr int SPI_MASTER_TASK_STACK_SIZE = 1024 * 4; // 4096 bytes for std::vector + logging
static constexpr int SPI_MASTER_TASK_PRIORITY = 12;
...
static spi_ll_clock_val_t s_forced_clock_reg {};
static bool s_use_forced_clock = false;

// ################### MY DEF ###############
static constexpr int SPI_MODE0 = 0;
static constexpr int HSPI = 1;
static constexpr int VSPI = 2;
static constexpr int FSPI = 0;
// ################## END MY DEF ############
...
struct spi_master_context_t
{
    ...
    spi_bus_config_t bus_cfg {
        .mosi_io_num = idf::GPIO_MOSI, // union with data0_io_num
        .miso_io_num = idf::GPIO_MISO, // union with data1_io_num
        .sclk_io_num = idf::GPIO_SCLK,
...
{
    if (s_use_forced_clock) {
        spi_ll_master_set_clock_by_reg(&GPSPI2, &s_forced_clock_reg);
    }
...
static void spi_master_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "spi_master_task init: clock_speed_hz=%d, clock_source=%d, flags=0x%x",
             ctx->if_cfg.clock_speed_hz, (int)ctx->if_cfg.clock_source, ctx->if_cfg.flags);
...
public:
    ...
    void captureSpiBusDmaQuad(bool useEnc2 = false){
        setContextForQuadDma(useEnc2);
        captureSpiBus();
    }

    void captureSpiBusDmaSingle(){
        setContextForSingleDma();
        captureSpiBus();
    }

    void captureSpiBus(){
        ESP_LOGI(TAG, "captureSpiBus: clock_speed_hz=%d, clock_source=%d, flags=0x%x, mode=%d, queue_size=%d",
                 this->ctx.if_cfg.clock_speed_hz, (int)this->ctx.if_cfg.clock_source,
                 this->ctx.if_cfg.flags, this->ctx.if_cfg.mode, this->ctx.if_cfg.queue_size);
        esp_err_t err = spi_bus_initialize(this->ctx.host, &this->ctx.bus_cfg, this->ctx.dma_chan);
        assert(err == ESP_OK);
        err = spi_bus_add_device(this->ctx.host, &this->ctx.if_cfg, &device_handle);
        ESP_LOGI(TAG, "captureSpiBus: spi_bus_add_device err=0x%x, device_handle=%p", err, (void*)device_handle);
        assert(err == ESP_OK);

        // Calculate correct clock register and enable pre_cb override
        int freq = this->ctx.if_cfg.clock_speed_hz;
        int eff_clk = spi_ll_master_cal_clock(APB_CLK_FREQ, freq, 128, &s_forced_clock_reg);
        s_use_forced_clock = true;
        ESP_LOGI(TAG, "captureSpiBus: forced clock reg=0x%08x, eff_clk=%d Hz",
                 *(uint32_t*)&s_forced_clock_reg, eff_clk);
    }

    void releaseSpiBus(){
        s_use_forced_clock = false;
        ESP_LOGI(TAG,"Master start releaseSpiBus!");
        esp_err_t ret = spi_bus_remove_device(device_handle);
        assert(ret==ESP_OK);
        ret = spi_bus_free (this->ctx.host);
        assert(ret==ESP_OK);
        ESP_LOGI(TAG,"Master end releaseSpiBus!");
    }



    static spi_device_handle_t  getDeviceHandle(){
    ...
    bool begin(uint8_t spi_bus = HSPI)
    {
        ...
        this->ctx.if_cfg.spics_io_num = idf::GPIO_CS;
        this->ctx.bus_cfg.sclk_io_num = idf::GPIO_SCLK;
        this->ctx.bus_cfg.mosi_io_num = idf::GPIO_MOSI;
        this->ctx.bus_cfg.miso_io_num =idf::GPIO_MISO;
#endif
    ...
    bool beginQuad(uint8_t spi_bus)
    {
        this->setContextForQuadDma();
        return this->initialize(spi_bus);
    }

  bool beginSingle(uint8_t spi_bus)
    {
        this->setContextForSingleDma();
        return this->initialize(spi_bus);
    }
    ...
    /// @brief execute one transaction using polling (no ISR, direct hardware access)
    ...
    size_t transferPolling(uint32_t flags, const uint8_t* tx_buf, uint8_t* rx_buf, size_t size)
    {
        spi_transaction_ext_t trans = generateTransaction(0, 0, 0, flags, 0, 0, size, tx_buf, rx_buf);
        spi_transaction_t *t = (spi_transaction_t*)&trans;
        esp_err_t err = spi_device_polling_transmit(device_handle, t);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "spi_device_polling_transmit failed: 0x%x", err);
            return 0;
        }
        return t->rxlength / 8;
    }
    ...
    size_t transfer(uint32_t flags, const uint8_t* tx_buf, uint8_t* rx_buf, size_t size, uint32_t timeout_ms = 0)
    {
        return this->transfer(0, 0, 0, flags, 0, 0, tx_buf, rx_buf, size, timeout_ms);
    }
    ...
    void setContextForQuadDma(bool useEnc2 = false){
        this->ctx.if_cfg.spics_io_num = idf::GPIO_CS;
        this->ctx.if_cfg.flags = SPI_DEVICE_HALFDUPLEX;
        this->ctx.if_cfg.clock_speed_hz = 10000000;
        this->ctx.if_cfg.cs_ena_pretrans = 2;
        this->ctx.if_cfg.cs_ena_posttrans = 0;
        this->ctx.if_cfg.mode = 0;
        this->ctx.if_cfg.clock_source = SPI_CLK_SRC_DEFAULT;
        this->ctx.bus_cfg.sclk_io_num = idf::GPIO_SCLK;
        if(useEnc2){
            // SDI=1: IC outputs data on ENC2 pins (sensor 2, contour C2)
            this->ctx.bus_cfg.data0_io_num = idf::GPIO_ENC2_0;
            this->ctx.bus_cfg.data1_io_num = idf::GPIO_MISO;
            this->ctx.bus_cfg.data2_io_num = idf::GPIO_ENC2_A;
            this->ctx.bus_cfg.data3_io_num = idf::GPIO_ENC2_B;
        } else {
            // SDI=0: IC outputs data on ENC1 pins (sensor 1, contour C1)
            this->ctx.bus_cfg.data0_io_num = idf::GPIO_ENC1_0;
            this->ctx.bus_cfg.data1_io_num = idf::GPIO_MISO;
            this->ctx.bus_cfg.data2_io_num = idf::GPIO_ENC1_A;
            this->ctx.bus_cfg.data3_io_num = idf::GPIO_ENC1_B;
        }
        this->ctx.bus_cfg.flags = SPICOMMON_BUSFLAG_QUAD;
        ESP_LOGI(TAG, "setContextForQuadDma: useEnc2=%d, data0=%d, data2=%d, data3=%d, clock=%d",
                 useEnc2,
                 useEnc2 ? idf::GPIO_ENC2_0 : idf::GPIO_ENC1_0,
                 useEnc2 ? idf::GPIO_ENC2_A : idf::GPIO_ENC1_A,
                 useEnc2 ? idf::GPIO_ENC2_B : idf::GPIO_ENC1_B,
                 this->ctx.if_cfg.clock_speed_hz);
    }

   

    void setContextForSingleDma(){
        this->ctx.if_cfg.spics_io_num  = idf::GPIO_CS;
        this->ctx.if_cfg.flags = 0;
        this->ctx.bus_cfg.sclk_io_num  = idf::GPIO_SCLK;
        this->ctx.bus_cfg.mosi_io_num  = idf::GPIO_MOSI;
        this->ctx.bus_cfg.miso_io_num  = idf::GPIO_MISO;
...
private:
    ...
    bool initialize(uint8_t spi_bus)
    {
        ...
        ESP_LOGI(TAG, "flags:%d", (int) this->ctx.if_cfg.flags);
        // create spi master task
        std::string task_name = std::string("spi_master_task_") + std::to_string(this->ctx.if_cfg.spics_io_num);
        int ret = xTaskCreatePinnedToCore(spi_master_task, task_name.c_str(), SPI_MASTER_TASK_STACK_SIZE, static_cast<void*>(&this->ctx), SPI_MASTER_TASK_PRIORITY, &spi_task_handle, 1);
        if (ret != pdPASS) {
    ...
    spi_transaction_ext_t generateTransaction(
        ...
        trans.base.rxlength =  8 * size; // set to same one with length
        trans.base.user = &this->cb_user_ctx; // user-defined callback and arg
        if(flags == SPI_TRANS_MODE_QIO){
            trans.base.tx_buffer = NULL;
            trans.base.length = 0;  // no tx phase in half-duplex QIO read mode
        }else {
            trans.base.tx_buffer = tx_buf;
        }
        trans.base.rx_buffer = rx_buf;//NULL; //(rx_buf == nullptr) ? NULL : rx_buf;
