...
| **Combined**      | Объединённая панель: HandTap + GraphView в виде 2×2         |
...
- **DMA Quad** (Options → ParallelSpiView) — двухканальный DMA-осциллограф
  с триггером, настройкой VC/SDI.
- **BLDC Motor** — управление двигателем через отдельный COM-порт
  (скорость, hall mode, калибровка электрического угла).
...
risc_programs/
  dma_asm/                       — программа для DMA Quad режима
